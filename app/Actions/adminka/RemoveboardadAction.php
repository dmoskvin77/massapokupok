<?php
/**
* Действие БО для удаления объявления
*/
class RemoveboardadAction extends AdminkaAction
{
	public function execute()
	{
		...
		if ($boardObject->status != BoardAd::STATUS_NEW)
			Adminka::redirect("manageboard", "Статус объявления не позволяет его отклонить");

		$boardObject->status = BoardAd::STATUS_DECLINED;
		...
		// отправим сообщение на е-майл "Объявление отклонено"
		$um = new UserManager();
		$userObj = $um->getById($boardObject->userId);
		$this->sendEmail($userObj, $boardObject);

		Adminka::redirect("manageboard", "Объявление отклонено");
	...
	{
		$shortTitle = "Ваше объявление отклонено";
		...
		$body = Adminka::prepareForMail(MailTextHelper::parse("boarddeclined.html", $vars));
