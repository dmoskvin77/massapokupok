<?php
/**
* Действие БО для восстановления заблокированного пользователя
*/
class RestoreUserAction extends AdminkaAction
{
	public function execute()
	{
		...
		if ($user->entityStatus != User::ENTITY_STATUS_BLOCKED)
			Adminka::redirect("manageusers", "Не подходящий статус");
		
		$user->entityStatus = User::ENTITY_STATUS_ACTIVE;
		$um->save($user);

		Adminka::redirect("manageusers", "Пользователь разблокирован");
