...
class UserplaceorderAction extends AuthorizedUserAction implements IPublicAction
{
	...
	public function execute()
	{
		...
		$rp = FilterInput::add(new IntFilter("rp", false, "Место в ряду"));
		$num = FilterInput::add(new IntFilter("num", false, "Ряд"));
		if (!$num)
			$num = 1;
	...
	private function redirectBackCover($message)
	{
		if ($this->curActor && $this->curActor->isOrg)
		{
			if ($this->curHeadid > 0 && $this->curLid > 0)
				Enviropment::redirect("orgviewzakupka/headid/".$this->curHeadid."#r".$this->curLid, $message);
			else if ($this->curHeadid > 0)
				Enviropment::redirect("orgviewzakupka/headid/".$this->curHeadid, $message);
			else
		...
		{
			if ($this->curHeadid > 0 && $this->curLid > 0)
				Enviropment::redirect("viewcollection/id/".$this->curHeadid."#r".$this->curLid, $message);
			else if ($this->curHeadid > 0)
				Enviropment::redirect("viewcollection/id/".$this->curHeadid, $message);
			else
				Enviropment::redirectBack($message);
		}
