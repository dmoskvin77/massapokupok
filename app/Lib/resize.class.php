...
class Resize {
    ...
    public function setNewSize($newHeight, $newWidth) {
		...
		$this->defHeight = $newHeight;
        $this->defWidth = $newWidth;
    }
    ...
    public function make() {
        ...
        if($this->getProportional() == 1) {
			...
			if ($this->width < $this->newWidth && $this->height < $this->newHeight)
				throw new Exception("Source file is too small");

            if($this->getProportionalFlag() == 'A') {
        ...
        switch ($this->imageType) {
            case 1:
                ...
                $newImg = imagecreate($this->defWidth, $this->defHeight);
				imagefill($newImg, 0, 0, 0xFFFFFF);
                break;
            case 2:
                $img    = imagecreatefromjpeg($this->getImage());
                $newImg = imagecreatetruecolor($this->defWidth, $this->defHeight);
				imagefill($newImg, 0, 0, 0xFFFFFF);
                break;
            case 3:
        ...
        try {
            imagecopyresized($newImg, $img, ($this->defWidth - $this->newWidth) / 2 , ($this->defHeight - $this->newHeight) / 2, 0, 0, $this->newWidth, $this->newHeight, $this->width, $this->height);
        }
        catch (Exception $e) {
