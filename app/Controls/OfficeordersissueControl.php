...
class OfficeordersissueControl extends AuthorizedUserControl
{
	...
	public function render()
	{
		...
		if ($code)
		{
			...
			// нам здесь нужно только то, что уже лежит в офисе
			$ordersByCode = array();
			$waitingOrdersByCode = $ohm->getByCode($this->ownerSiteId, $this->ownerOrgId, $code);
			$oom = new OfficeOrderManager();
			$waitingOfficeOrdersIds = $oom->getWaitingOfficeOrdersIds($this->ownerSiteId, $this->ownerOrgId);
			if (count($waitingOrdersByCode) && count($waitingOfficeOrdersIds))
			{
				foreach ($waitingOrdersByCode AS $waitingItem)
				{
					if (in_array($waitingItem->id, $waitingOfficeOrdersIds))
						$ordersByCode[] = $waitingItem;
				}
			}
			...
			if (count($ordersByCode))
			{
				...
				// выдача закупок любого организатора
				$zheadList = $zhm->getByIds($headIds);
