...
try
{
	...
	try
	{
        // вычислить комиссию за клики (pay per order) для сайтов
        $osm = new OwnerSiteManager();
        $ownerSitesList = $osm->getAll();
        if (count($ownerSitesList)) {
            foreach ($ownerSitesList AS $ownerSite) {
                $qsm->savePlaceTask("calccommsite_" . $ownerSite->id, null, null, null);
            }
        }

        // вычислить комиссию за клики (pay per order) для оргов
        $oom = new OwnerOrgManager();
        $ownerOrgList = $oom->getAll();
        if (count($ownerOrgList)) {
            foreach ($ownerOrgList AS $ownerOrg) {
                $qsm->savePlaceTask("calccommorg_" . $ownerOrg->id, null, null, null);
            }
        }
